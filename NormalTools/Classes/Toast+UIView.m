...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if(image != nil) {
        ...
        imageView.frame = CGRectMake(0, 0, CSToastImageViewWidth, CSToastImageViewHeight);
    }
    //对imageView的size赋值
    CGFloat imageWidth, imageHeight, imageLeft ,imageTop;
    // the imageView frame values will be used to size & position the other views
    if(imageView != nil) {
        imageWidth = imageHeight = CSToastImageViewHeight;
    ...
    if (title != nil) {
        ...
        CGSize maxSizeTitle = CGSizeMake((self.bounds.size.width * CSToastMaxWidth), self.bounds.size.height * CSToastMaxHeight);
        NSDictionary *attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:CSToastFontSize]};
    ...
    CGFloat titleWidth, titleHeight, titleTop;
    if(titleLabel != nil) {
        titleWidth = titleLabel.bounds.size.width;
        titleHeight = titleLabel.bounds.size.height;
        titleTop = imageTop + imageHeight + CSToastVerticalPadding;
    } else {
        titleTop = imageTop + imageHeight;
...
//        CGSize expectedSizeMessage = [message sizeWithFont:messageLabel.font constrainedToSize:maxSizeMessage lineBreakMode:messageLabel.lineBreakMode]; 
    ...
    CGFloat messageWidth, messageHeight, messageLeft = 0.0, messageTop;

    if(messageLabel != nil) {
        messageWidth = messageLabel.bounds.size.width;
        messageHeight = messageLabel.bounds.size.height;
        messageTop = titleTop + titleLabel.height + CSToastVerticalPadding;
    } else {
        messageTop = titleTop + titleLabel.height;
        messageWidth = messageHeight = messageLeft = 0.0;
    }
    

    CGFloat longerWidth = MAX(titleWidth, messageWidth);
    // wrapper width uses the longerWidth or the image width, whatever is larger. same logic applies to the wrapper height
    CGFloat wrapperWidth = MAX((imageWidth + (CSToastHorizontalPadding * 2)), (CSToastHorizontalPadding + longerWidth + CSToastHorizontalPadding));
    ...
    if(imageView != nil) {
        imageView.frame = CGRectMake(wrapperView.centerX - imageWidth/2., imageTop, imageWidth, imageHeight);
        [wrapperView addSubview:imageView];
    }
    
    if(titleLabel != nil) {
        titleLabel.frame = CGRectMake(wrapperView.centerX - titleWidth/2., titleTop, titleWidth, titleHeight);
        [wrapperView addSubview:titleLabel];
    }
    
    if(messageLabel != nil) {
        messageLabel.frame = CGRectMake(wrapperView.centerX - messageWidth/2., messageTop, messageWidth, messageHeight);
